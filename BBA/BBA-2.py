...
class R2AFixed(IR2A):
    ...
    def handle_segment_size_request(self, msg):
        ...
        if not (len(self.buffer) > 0):
            ...
            print("startup!!!")
            result = next((tup for tup in self.buffer[::-1] if tup[0] < self.buffer[-1][0] - 1), (0,0))
            V = (self.buffer[-1][1] - result[1])
            if (V > 4) and (self.last_request < 20):
                self.last_request += 1
                msg.add_quality_id(self.qi[self.last_request])
            elif (V < 1) and (0 < self.last_request < 20):
                self.last_request -= 1
                msg.add_quality_id(self.qi[self.last_request])
            else:
                msg.add_quality_id(self.qi[self.last_request])
